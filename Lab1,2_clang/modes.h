...
std::string cipher, cipherEncoded, recovered;
...
std::string ECB_Encrypt(std::string plain, CryptoPP::byte *key) {
    ...
    cipherEncoded.clear();
    StringSource(cipher, true, new HexEncoder(
        new StringSink(cipherEncoded)
        )
    );
...
std::string AES_Decrypt(int mode, std::string cipher, CryptoPP::byte *key, CryptoPP::byte *IV = nullptr) {
    switch (mode) {
        case 1:
            return ECB_Decrypt(cipher, key);
            break;
        case 2:
            return CBC_Decrypt(cipher, key, IV);
            break;
        case 3:
            return CFB_Decrypt(cipher, key, IV);
            break;
        case 4:
            return OFB_Decrypt(cipher, key, IV);
            break;
        case 5:
            return CTR_Decrypt(cipher, key, IV);
            break;
        case 6:
            return XTS_Decrypt(cipher, key, IV);
            break;
        case 7:
            return CCM_Decrypt(cipher, key, IV);
            break;
        case 8:
            return GCM_Decrypt(cipher, key, IV);
            break;
    }
}
